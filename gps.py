...
def get_gps_data(raw_data, date_string):
    ...
    # Checks if the current message is a "$GPGGA" message", and that there is a valid date_string
    # If so, it processes the message to extract the required information
        ...
        # Finally the change_datetime function is called which changes the current system date and time
...
while True:
    ...
    if raw_data[0:6].decode() is '$GPRMC':
        date_string = raw_data.decode().split(',')[9]

    # If the date_string variable has a valid date in it, run get_gps_data to gather the necessary data
    # and write it to file.  This ensures that the get_gps_data function is not run until at least one $GPRMC message
    # has been collected, to get a valid date.
    if raw_data[0:6].decode() is '$GPGGA' and date_string is not None:
